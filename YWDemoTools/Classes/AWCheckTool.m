...
@implementation AWCheckTool

+ (BOOL)priceFormat:(NSString *)price{
    if (price.length > 0) {
        NSString *stringRegex = @"(\\+|\\-)?(([0]|(0[.]\\d{0,8}))|([1-9]\\d{0,8}(([.]\\d{0,8})?)))?";
...
+ (NSDictionary *)dictionaryWithJsonString:(NSString *)jsonString
{
    ...
    if(err) {
        NSLog(@"----------json-->dictionary 解析失败：%@",err);
        return nil;
    }
    return dic;
}

#pragma mark 字典转化字json符串
...
+ (BOOL)checkAccount:(NSString *)str {
    NSString *regex = @"^[a-z][a-z0-9.-]{9,20}$";
    //创建谓词对象并设定条件的表达式
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    // 对字符串进行判断
    return [predicate evaluateWithObject:str];
